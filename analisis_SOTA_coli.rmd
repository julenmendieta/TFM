...
library("impute")
dir = "/media/julen/TOSHIBA_EXT/datos/paper/coli"
setwd(dir)

matriz = readRDS(file="datos_cargar/matriz_coli.rds")
fin="_coli"
...
# Me he descargado de http://www.ecogene.org/?q=ecodownload/crossref los b.numbers asociados a todos mis genes
idConv = read.csv(file="datos_cargar/coli_symblBname.txt", sep="\t")

# Buscamos enriquecimineto GO con Gprofiler
enriquecimientoKEGG <- function(listaGenes, longSinGO="?", especie="eco", idConv1=idConv) {
...
#     idConv1 = read.csv(file="datos_cargar/coli_symblBname.txt", sep="\t")
    ...
    # Esta funcion olo acepta b numbers de identificador (puede que tambien locus names)
    salida = enrichKEGG(listaGenes1, organism = especie, pvalueCutoff = 0.05, pAdjustMethod = "fdr", 
              qvalueCutoff = 0.1, use_internal_data = FALSE)
...
enriquecimientoGO <- function(listaGenes) {
    ...
    xx1 <- as.list(org.EcK12.egALIAS2EG)
    ...
    entrzSymb = as.list(org.EcK12.egSYMBOL)
    ...
    for(ont in ontologies) {
        params <- new("GOHyperGParams" , geneIds = catego1,
                    ...
                    conditional =F , testDirection = "over" , annotation ="org.EcK12.eg")
...
plotear <- function(DatosFilt, clu, clu_comp, ran1 = -0.1, ran2 = 1.1, go = TRUE, seguir = TRUE, valor_silueta, kegg = TRUE) {
    ...
    if (kegg == TRUE) {
        # Reducimos los decimales que aparecen para los GO en SOTA
        options("scipen"=-100, "digits"=3)
        salida = enriquecimientoKEGG(rownames(DatosFilt), especie="eco")
...
escala = nrow(Matriz_6) / 30
# Si queremos que el arbol sea recursivo, reursivo=TRUE
recursivo=FALSE
# Si queremos busqueda de GO
busqueda_go = FALSE
# Si queremos busqueda de KEGG ################(No consigo desactivarla desde aqui)###############
busqueda_kegg = FALSE
