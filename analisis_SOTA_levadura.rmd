...
library("impute")
setwd("/media/julen/TOSHIBA_EXT/datos/paper/levadura")
...
SOTA <- function(Matriz_6, ciclos, escala, recursivo) {
...
global_SOTA <- function(Matriz_6, clusters_test = "inicio", reanalizar = "inicio", s.sota = 0, escala, clu_comp = "", fin="", nodos=c(), record_silueta = "Silueta", recursivo=TRUE) {
  if(clu_comp != "") {
    # Si en algun momento hay que acelerar el proceso esto hay q moverlo
    dir.create(file.path(paste("salida", fin, sep="")), showWarnings = FALSE)
    #
    salida = paste("salida", fin, "/", clu_comp, sep = "")
  ...
  for(clu in clusters_test) {
    if (!(clu %in% reanalizar)) {
      ...
      plotear(DatosFilt, clu, clu_comp, go=TRUE, valor_silueta=valor_silueta)
  ...
  for (ar in 1:length(barbol)) {
    ...
    # Si la longitud es mayor, hay que abrir parentesis
      ...
      if (paste(strsplit(ent, "\\.")[[1]][1:(longi2 - 1)], collapse=".") != 
        ...
        salida = paste(salida, ")", nodo, ":", length(strsplit(nodo, "\\.")[[1]]), ".0", ",",paste(rep("((", sumar), collapse=""), barbol[ar], ":", longi, ".0", sep="")
...
# Vamos a analizar los grupos con mas de 10 genes --> n = 10
escala = nrow(Matriz_6) / 20

arbol = global_SOTA(Matriz_6, escala = escala, fin=fin, recursivo=FALSE)
```
...
# Revision de clusters y modificaciones

```{r echo=FALSE, message = FALSE, include = FALSE}
# Cambiamos funciones para que se ajusten a la revision y generamos otras nuevas
global_SOTA <- function(Matriz_6, clusters_test = "inicio", reanalizar = "inicio", s.sota = 0, escala, clu_comp = "", fin="", nodos=c()) {
  for(clu in clusters_test) {
    if (!(clu %in% reanalizar)) {
      pos = s.sota$clust == clu
      DatosFilt = Matriz_6[pos,]
      plotear(DatosFilt, clu, clu_comp)
      nodos = append(nodos, paste(as.character(clu_comp), ".", as.character(clu), sep = ""))
    }
    ## Vamos a dividir los clusters mas grandes
    else {
      ...
      # a el total de genes analizados dividido por las 25 divisiones
      ciclos = round(nrow(Matriz_filtr)/escala, digits = 0)
      lista_sota = SOTA(Matriz_filtr, ciclos, escala)
      # Con esto se hace el seguimiento del origen de cada cluster
      if (clu_comp != "") {
        clu_comp2 = paste(as.character(clu_comp), ".", clu, sep = "")
      }
      ...
      nodos = global_SOTA(Matriz_filtr, lista_sota[["clusters_test"]], lista_sota[["reanalizar"]], lista_sota[["sota"]], 
                          escala = escala, clu_comp = clu_comp2, fin=fin, nodos=nodos)
    }
  }
  return(nodos)
}

# Funcion para juntar cluster en los que hemos visto que salia el mismo GO como mas significativo
juntaCluster <- function(juntar, cabeceras = c("RA", "TR", "RS", "PA", "TLRi", "PS" ), fin = "") {
  matriz_temp = matrix(ncol = length(cabeceras))
  colnames(matriz_temp) = cabeceras
  for (jun in juntar) {
    if (is.na(jun)) {
      next
    }
    # Primero sacamos la rama de origen, de donde sacaremos los genes que pertenecen al cluster indicado
    entrada = paste("salida", fin, "/", jun, sep = "")
    entrada = strsplit(entrada, "\\.")[[1]]
    clu = paste(entrada[length(entrada)], collapse = ".")
    entrada = paste(entrada[1:(length(entrada) - 1)], collapse = ".")
    s.sota = readRDS(entrada)
    pos = s.sota$clust == clu
    DatosFilt = s.sota$data[pos,]
    matriz_temp = rbind(matriz_temp, DatosFilt)
  }
  matriz_temp = matriz_temp[-1,]
  juntar = juntar[!is.na(juntar)]
  plotear(matriz_temp, clu = "", clu_comp = paste(juntar, collapse = "_")) 
}
```
...
#```{r echo=FALSE}
j1 = c("0.5", "0.6", "0.7")
grupos <- matrix(c(j1),nrow=1,byrow=TRUE)


for (jun in 1:nrow(grupos)) {
  juntaCluster(juntar = grupos[jun,], fin = fin)
}
```
